<template>
  <div class="app-container">
    ...
    <el-dialog title="修改信息" :visible.sync="dialogFormVisible" width="500px">
      ...
      >
        <!-- 审核状态 -->
        <el-form-item label="审批状态" prop="leaveState">
          <el-select
            v-model="temp.leaveState"
            class="filter-item"
            placeholder="请选择"
          >
            <el-option
...
export default {
  ...
  methods: {
    ...
    //打开更新对话框
    handleUpdate(row) {
      const { id, leaveState } = row;
      const record = {
        id,
        leaveState,
      };
      this.temp = record; // copy obj
    ...
    updateData() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          this.$store.dispatch("leave/updateRecord", tempData).then(() => {
            this.getList();
    ...
    //触发删除操作
    handleDelete(row) {
      this.$store.dispatch("leave/deleteRecord", { id: row.id }).then(() => {
        this.getList();
        this.$notify({
          title: "成功",
          message: "删除成功",
